program main
  ...
  !
    ...
    integer ( kind = 4 ) seed
    real ( kind = 8 ) thresh
    character(len=10)::iter,tres,newn
  ...
  !escribir el entero en una string
  !  thresh =0.02D+00
  !  iterations=600
    WRITE(iter,'(i0)')   iterations
    WRITE(tres,'(F10.2)')   thresh 
  newn= trim(iter)// "_"// trim(adjustl(tres)) 
  
  
    !
    call plot_file ( m, n, c1, 'Initial Configuration', 'ising_2d_initial_' // trim(adjustl(newn)) // '.txt', &
      'ising_2d_initial_' // trim(adjustl(newn))  // '.png' )
  !
  !  Do the simulation.
  !
  ...
  !
    call plot_file ( m, n, c1, 'Final Configuration', 'ising_2d_final_'// trim(adjustl(newn))  // '.txt', &
