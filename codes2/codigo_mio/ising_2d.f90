program main
  ...
  real(kind=8),dimension(10000):: tu
  real(kind = 8), dimension(-4:4) :: h,q
  real(kind = 8) :: T  
  integer:: M, M0, mc ,contador  ,nm, ni,nmlim,imedida
  integer :: L, N , i ,iseed, j ,ij, ib, im , iest
  real(kind = 8) :: c, rm, rm2, rm1 , rm0 , tau ,error , chi ,uaux, U, U2, uaux2,cv, rm4,cbinder,rm2aux
    REAL(kind=8)::nlctden,rminicial,rmest,taunl
  ...
  ! character(len=10)::temp,newn
  REAL(kind=8)::alpha,qm
  ...
  M0 = 1000 !inital thermalization steps
  mc = 10 !monte carlo steps per run 
  nm=30  ! number of measures
  ni=10  ! initial measure
...
rminicial=real(abs(sum(s)))/M

PRINT*, "RM INICIAL:", rminicial
...
!!! TAKE MEASURES FOR EACH VALUE OF imedia 
          ...
          if (im==imedida) then 
              ...
              imedida=INT(alpha**(contador-1)*ni)
              ! PRINT*, im
      ...
      if (abs(qm-1)<=0.05) then
        rmest=rm/M
        nlctden=rminicial-rmest  
        iest=im  
        PRINT*, im
      end if 
  ...
  taunl=1.d00
  DO i=1,iest
  taunl= taunl + (tu(i)-rmest)/nlctden

  END DO 
    PRINT*, " N-L correlation time:",  taunl

  ! close(88)    
  ! close(66)
END IF

deallocate(s, n1, n2, n3, n4)


contains 

function initial_state(nn,dran_u) result(ss) 
   implicit none
  REAL(kind = 8),   EXTERNAL :: dran_u 
  integer, dimension(nn)  :: ss
  integer ::a,nn
